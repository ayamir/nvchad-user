...
local function get_git_diff_files()
  ...
  local tracked_files = git_systemlist({
    string.format("git -C %s diff --name-only HEAD -- 2>/dev/null", git_root_escaped),
  })
...
vim.api.nvim_create_user_command("OpenGitDiffFiles", function()
